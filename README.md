...
> Folder `android/` sengaja tidak disimpan di repository. Folder native Android dibuat otomatis oleh `npx cap add android` saat proses build. Ini menjaga repository tetap ringan dan memungkinkan Android project diregenerasi dari konfigurasi Capacitor.
...
- JDK sesuai toolchain Android Studio yang terpasang
...
7. Upload `app-debug.apk` sebagai GitHub Actions artifact
...
APK yang dihasilkan workflow saat ini adalah **debug APK** untuk pengujian. Untuk distribusi ke pengguna/Google Play, buat signing keystore dan build release (`assembleRelease`/bundle release) dengan konfigurasi signing yang aman.
